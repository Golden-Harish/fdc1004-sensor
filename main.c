...
#include <zephyr/drivers/sensor/fdc1004.h>
#include <stdio.h>
...
#define DEVICE_NODE DT_COMPAT_GET_ANY_STATUS_OKAY(ti_fdc1004)
...
int main(void)
{
	...
	while (1) {
...
#endif
		/* Read capacitance for each channel */
		base = SENSOR_CHAN_FDC1004_CAPACITANCE_CH0;
		for (i = 0; i < num_ch; i++) {
		...
		ret = pm_device_action_run(dev, p_action);
		pm_info(p_action, ret);
#elif CONFIG_FDC1004_TRIGGER_NONE
		k_sleep(K_MSEC(100));
#endif
	}
}
